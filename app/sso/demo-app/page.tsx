...
function SatelliteAppContent() {
  const searchParams = useSearchParams();
  const router = useRouter();

  const appKey = searchParams.get("app") || "skillhub";
  ...
  // Verify SSO Token with Central BPVP SSO verification endpoint
  useEffect(() => {
    async function verify() {
      ...
      try {
        const res = await fetch(`/api/sso/verify?token=${encodeURIComponent(ssoToken)}`);
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col justify-between">
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {verified && tokenData ? (
              ...
              <Link
                href={`/sso/kemnaker?callbackUrl=${encodeURIComponent(`/sso/demo-app?app=${appKey}`)}`}
        ...
        {loading ? (
          <div className="bg-white rounded-2xl p-12 text-center shadow-md border border-slate-200 space-y-3">
            ...
            <h3 className="text-base font-bold text-slate-800">
              Memverifikasi Token SSO dengan Basis Data Pegawai BPVP...
            </h3>
            <p className="text-xs text-slate-500 max-w-md mx-auto">
              Sistem sedang memeriksa integritas tanda tangan kriptografis HMAC-SHA256 dan memastikan status keaktifan pegawai.
            </p>
          </div>
        ) : verified && tokenData ? (
          /* SUCCESS STATE: SEAMLESS LOGIN VERIFIED */
          <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-emerald-200">
                    Tanpa Password Ulang (Single Sign-On Aktif)
                  </span>
                </div>
                ...
                <p className="text-xs sm:text-sm text-emerald-100 max-w-2xl leading-relaxed">
                  Anda berhasil masuk ke aplikasi <span className="font-bold text-amber-300">{currentApp.nama}</span> secara otomatis menggunakan Token Kredensial Terpusat dari <span className="font-bold underline">SIMPEG BPVP Banda Aceh</span>.
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              {/* Cross-App Navigation Card (Pindah Antar Aplikasi BPVP) */}
              <div className="md:col-span-2 bg-white rounded-2xl p-6 shadow-md border border-slate-200 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-amber-700 font-semibold bg-amber-50 px-2 py-0.5 rounded border border-amber-200">
                    Token Otomatis Diteruskan
                  </span>
                </div>
                <p className="text-xs text-slate-600">
                  Pilih aplikasi ekosistem BPVP di bawah ini untuk berpindah langsung dengan sesi kredensial yang sama:
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500">Kembali ke Dashboard Pegawai</p>
                      </div>
                    </div>
                  ...
                  {Object.values(APPS_META)
                    ...
                    .map((a) => (
                      <Link
                        key={a.id}
                        href={`/sso/demo-app?app=${a.id}&sso_token=${encodeURIComponent(ssoToken)}`}
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                  <button
                    ...
                    className="text-xs font-semibold text-[#003399] hover:underline flex items-center gap-1.5"
                  >
            ...
            {showInspector && (
              <div className="bg-slate-900 text-slate-200 rounded-2xl p-6 shadow-xl border border-slate-800 space-y-4">
                ...
                <p className="text-[11px] text-slate-400">
                  Token ini diterbitkan oleh Manajemen Pegawai BPVP dan hanya berlaku untuk pegawai yang berstatus aktif di database.
                </p>
              </div>
            )}
          </div>
        ) : (
          /* REJECTION STATE: OUTSIDER / INACTIVE / MISSING TOKEN */
          <div className="max-w-2xl mx-auto bg-white rounded-2xl p-8 shadow-xl border border-red-200 text-center space-y-6">
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
              <Button
                onClick={() => router.push(`/sso/kemnaker?callbackUrl=${encodeURIComponent(`/sso/demo-app?app=${appKey}`)}`)}
                className="w-full sm:w-auto bg-[#003399] hover:bg-[#002266] text-white font-bold text-xs h-10 px-6 shadow-md"
              >
                Masuk via Portal SSO Kemnaker / BPVP
              </Button>
              <Button
                variant="outline"
                onClick={() => router.push("/")}
                className="w-full sm:w-auto text-xs h-10 px-5"
              >
      ...
      <footer className="bg-slate-900 text-slate-400 text-[11px] py-4 px-6 text-center border-t border-slate-800 mt-8">
        ...
        <p className="mt-0.5 text-slate-500">
          Terintegrasi penuh dengan Single Sign-On (SSO) Berbasis Manajemen Pegawai &amp; Standar Keamanan Pemerintah
        </p>
      </footer>
    </div>
  );
...
export default function SatelliteAppPage() {
  return (
    <Suspense
      fallback={
